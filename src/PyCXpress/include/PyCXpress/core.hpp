...
namespace PyCXpress {
    ...
    class PYCXPRESS_EXPORT            Buffer {
        ...
        template <typename T> static py::array __to_array(const std::vector<size_t> &shape,
            ...
            *stride.rbegin() = sizeof(T);
            ...
            max_size = real_size;
            return py::array_t<T>{shape, std::move(stride), (T *)(data), py::none()};
        }
    ...
    public:
        ...
        void reset() { m_array = m_converter({m_size / m_itemsize}, m_data, m_size); }
    ...
    public:
        ...
        BufferPtr set_buffer(const std::string &name, const std::vector<size_t> &shape) {
        ...
        std::pair<BufferPtr, std::vector<size_t>> get_buffer(const std::string &name) {
            auto &buf   = m_buffers.at(name);
            ...
            return std::make_pair(std::make_pair(array.request().ptr, nBytes), std::move(shape));
        }
    ...
    private:
        void initialize(const char *module, const char *name) {
            ...
            for (auto d = spec.begin(); d != spec.end(); d++) {
                ...
                auto       buf  = m_buffers.insert(
                    {name, Buffer{meta[2].cast<size_t>(), meta[1].cast<std::string>()}});
                if (meta[3].cast<bool>()) {
    ...
    public:
        ...
        PythonInterpreter(PythonInterpreter &&other) noexcept { other.is_valid = false; }
        ...
        void initialize(bool init_signal_handlers = true, int argc = 0,
            ...
            py::initialize_interpreter(init_signal_handlers, argc, argv, add_program_dir_to_path);
            is_valid = true;
        }
        void finalize() {
            m_models.clear();
        ...
        Model &create_model(const std::string &path, const std::string &name = "default") {
            if (!is_valid) {
                initialize();
            }
            if (m_models.find(name) == m_models.end()) {
                ...
                std::cerr << "Warning: Model with name " << name << " already exists" << std::endl;
            }
